...
int main (int argc, char **argv)
{
  ...
  /*
    ...
    sur les polynomes
  */
  float res = puiss_float(2.0,2);
  float r = eval_polynome(p1,1);
  p_polyf_t p_a = multiplication_polynome_scalaire(p1,2.0);
  p_polyf_t p_b = multiplication_polynomes(p1,p2);
  p_polyf_t p_c = puissance_polynome(p1,2);
  p_polyf_t p_d = composition_polynome(p1,p2);

  printf("res_puiss = %f\n", res);
  printf("%s(1) = %f\n",argv [1], r);
  printf("\n2* %s = ",argv [1]);
  ecrire_polynome_float (p_a) ;
  printf("\n%s * %s = ",argv [1],argv [2]);
  ecrire_polynome_float (p_b) ;
  printf("\n%s^2 = ",argv [1]);
  ecrire_polynome_float (p_c) ;
  printf("\n%s o %s = ",argv [1],argv [2]);
  ecrire_polynome_float (p_d) ;
  printf("\n");
  detruire_polynome (p_a) ;
  detruire_polynome (p_b) ;
  detruire_polynome (p_c) ;
  detruire_polynome (p_d) ;
